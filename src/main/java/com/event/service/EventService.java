...
public class EventService {
	...
	public Map<String,List<Event>> groupEvents(){
		List<Event> pendingList = eventRepo.findByStatus("Pending");
		List<Event> completedList = eventRepo.findByStatus("Completed");
		
		List<Event> overdueList = eventRepo.findAll().stream()
				.filter(event -> event.getDateTime().isBefore(LocalDateTime.now()) && !event.getStatus().equals("Completed"))
